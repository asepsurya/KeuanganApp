    ...
    <style>
        ...
        <form action="{{ route('action.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="px-2 py-1 mb-4 flex items-center justify-between">
                ...
                <button class="px-3 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition" @click="toggle">
                    Simpan Perubahan
                </button>
            </div>
            @if ($errors->any())
                ...
                <div class="">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="py-4 px-5 mb-3 bg-white rounded-lg border border-black/10 relative dark:bg-white/5">
                            ...
                            <input type="text" name="kode_produk" id="kode_produk" class="form-input" readonly value="{{ $item->kode_produk }}" />
                        </div>

                        <!-- Nama Produk -->
                        <div class="py-4 px-5 mb-3 bg-white rounded-lg border border-black/10 relative dark:bg-white/5">
                            <label class="block mb-1 text-xs text-black/40 dark:text-white/40">Nama Produk</label>
                            <input type="text" name="nama_produk" placeholder="Nama Produk" class="form-input" value="{{ $item->nama_produk }}" />
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-7">
                            ...
                            <div class="py-4 px-5 mb-3 bg-white rounded-lg border border-black/10 relative dark:bg-white/5">
                                ...
                                <input type="number" name="harga" placeholder="Harga Produk" class="form-input" value="{{ $item->harga }}"  />
                            </div>

                            <!-- Stok -->
                            <div class="py-4 px-5 mb-3 bg-white rounded-lg border border-black/10 relative dark:bg-white/5">
                                <label class="block mb-1 text-xs text-black/40 dark:text-white/40">Stok Produk</label>
                                <input type="number" name="stok" placeholder="Stok Produk" class="form-input" value="{{ $item->stok }}" />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-7">
                            ...
                            <div class="py-4 px-5 mb-3 bg-white rounded-lg border border-black/10 relative dark:bg-white/5">
                                ...
                                <select name="status" class="status form-select w-full">
                                    ...
                                    <option value="out_of_stock" {{ $item->status == 'out_of_stock' ? 'selected' : '' }}>Habis Stok</option>
                        ...
                        <div class="py-4 px-5 bg-white rounded-lg border border-black/10 relative dark:bg-white/5 mb-3">
                            ...
                            @if($item->gambar)
                                <img id="imgPreview" src="{{ asset('/storage/' . $item->gambar) }}" alt="Preview"
                                    class="mt-4 w-full rounded-lg" onerror="this.onerror=null;this.src='{{ asset('assets/images/404.jpg') }}';" />
                            @else
                        ...
                        <div class="py-4 px-5 bg-white rounded-lg border border-black/10 relative dark:bg-white/5 mb-3">
                            ...
                            <select id="select-kategori" name="kategori" class="form-select w-full">
                                ...
                                @foreach ($category as $item2)
                                    <option value="{{ $item2->id }}" {{ $item->kategori == $item2->id ? 'selected' : '' }}>{{ $item2->name }}</option>
                                @endforeach
